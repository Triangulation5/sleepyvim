...
for _, m in ipairs({
    ...
    { "n", "<leader>wz", function() require("mini.misc").zoom() end, "Zen Mode" }, { "n", "<leader>wr", function() require("mini.misc").resize_window() end, "Resize Window" }, { { "n", "x" }, "<C-d>", "<C-d>zz", "Scroll Down" }, { { "n", "x" }, "<C-u>", "<C-u>zz", "Scroll Up" }, { "n", "n", "nzzzv", "Next search result" }, { "n", "N", "Nzzzv", "Previous search result" },
    ...
    { "n", "x", '"_x', "Cut (no yank)" }, { "n", "<M-t>", function() vim.cmd('botright split term://powershell') end, "PowerShell" }, { "n", "<leader>ti", function() vim.notify(string.format("Time: %s | Date: %s | %s", os.date("%I:%M:%S %p"), os.date("%Y-%m-%d"), os.date("%A")), vim.log.levels.INFO, { title = "Clock" }) end, "Time" },
    { "n", "<leader>g", function() if vim.fn.executable("glow")==1 then local buf=vim.api.nvim_create_buf(false,true); local w,h=math.floor(vim.o.columns*0.9),math.floor(vim.o.lines*0.85); local r,c=math.floor((vim.o.lines-h)/2),math.floor((vim.o.columns-w)/2); local win=vim.api.nvim_open_win(buf,true,{relative="editor",width=w,height=h,row=r,col=c,style="minimal",border="rounded"}); vim.cmd("terminal powershell glow") else vim.notify("glow not found",vim.log.levels.WARN) end end, "Glow" },
