...
-- Add this for differnt menu configurations in blink.cmp
-- blink.cmp completion menu layout
...
-- VS Code style layout (if your evil):
draw = {
  columns = {
    { "label", gap = 1 },
    { "kind_icon" },
...
-- Add this for a dynamic title in your terminal:
titlestring = "%t - Nvim"

-- Add this for no case sensitive searching:
-- ignorecase = true → search is case-insensitive
-- smartcase = true → becomes case-sensitive if your query contains any uppercase letters
vim.opt.ic = true
vim.opt.scs = true
