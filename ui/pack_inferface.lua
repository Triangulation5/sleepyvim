...
local function build_content()
  ...
  local header = (" vim.pack  %d plugins  %d updates"):format(
    #state.plugins,
    #state.pending
  )
  ...
  local help =
    " [r] refresh  [u] update plugin  [U] update all  [Enter] details  [q] close"
  ...
  local function add_plugin(plugin, pending)
    ...
    local revs = pending
        and (" " .. short_rev(plugin.rev) .. " -> " .. short_rev(plugin.rev_to))
    ...
    if hash_start then add_hl(row, hash_start - 1, #line, "PackFloatHash") end
      ...
      if pending then
        ...
        elseif #commits == 0 then
          ...
          for i = 1, limit do
            ...
            if hash then add_hl(commit_row, 4, 4 + #hash, "PackFloatHash") end
          end
  ...
  if #state.pending == 0 then
    add(
      state.checking and checking_label() or "  no pending updates",
      "PackFloatMuted"
    )
  else
